...
class LoggingInitializer : public absl::LogSink {
 public:
  LoggingInitializer() { absl::AddLogSink(this); }
 ...
 private:
  void Send(const absl::LogEntry&) override {
    absl::call_once(once_, [this]() {
      ...
      if (auto severity =
              LogLevelStrToInt(std::getenv("TF_CPP_MIN_LOG_LEVEL"))) {
...
absl::NoDestructor<LoggingInitializer> g_initializer;
